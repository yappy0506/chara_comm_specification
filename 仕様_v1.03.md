...
- [キャラ演技LLM](#キャラ演技llm)
  ...
  - [2. アプリの諸シーケンス](#2-アプリの諸シーケンス)
    - [2.1. 起動シーケンス](#21-起動シーケンス)
      - [2.1.1. 初回起動シーケンス](#211-初回起動シーケンス)
      - [2.1.2. 通常起動シーケンス](#212-通常起動シーケンス)
    - [2.2. アプリケーションのシーケンス](#22-アプリケーションのシーケンス)
  - [3. システム詳細](#3-システム詳細)
...
- 音性出力(TTSサーバ)：Bert-VITS2 JP-Extra
  アプリケーションが設定、有効化をする。
...
### 2.1. 起動シーケンス
以下はApplicationの実行までに自動で実施されるものとする。

#### 2.1.1. 初回起動シーケンス

1. pythonの仮想環境の有効化
2. pythonにおけるアプリケーションに必要なモジュールのインストール
3. TTSサーバの有効化
   1. (インストールされてない場合は)Style-Bert-VITS2JP-Extraのインストール
   2. Style-Bert-VITS2JP-Extraの設定、起動
   3. APIサーバの立ち上げ
4. Applicationの実行
